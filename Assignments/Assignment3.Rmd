...
This exercise should be answered using the `Default` data set, which is
part of the `LSLR` package. If you don't have it installed already you
can install it with
```{r}
#install.packages("ISLR")
```
...
a.  Comment on the estimated standard errors obtained using the
    `summary()` function on the first model and the estimated standard
    errors you found using the above code.
